...
def read_annotated_pos_data(dialign_output, turn_info_path):
   ...
   for f in os.listdir(dialign_output):
      if f.endswith('_tsv-lexicon.tsv') and not f.startswith('.'):
         ...
         files_parts = filepath.split('/')[-1].split('_')
         fribble_ID = files_parts[3]
         pair_name = '_'.join(files_parts[0:3])
...
def prepare_dialogue_shared_expressions_and_turns_info_pseudo(dialign_output, turn_info_path, self_repition=False):
    if self_repition:
...
if __name__ == "__main__":
   # code to set up the paths
    dialign_output = '/Users/esamghaleb/important_surfdrive/Research/CDM/Projects/LinguisticAlignmentFullGestures/code/dialign/output_targets_riws_lemma_pseudo'
    turn_info_path = '/Users/esamghaleb/important_surfdrive/Research/CDM/Projects/LinguisticAlignmentFullGestures/code/dialign/targets_riws_lemma_pseudo/'
